...
RowLayout {
    ...
    property var suspend: ["Сон", config.TranslateSuspend || textConstants.suspend, sddm.canSuspend]
    property var reboot: ["Перезагрузка", config.TranslateReboot || textConstants.reboot, sddm.canReboot]
    property var shutdown: ["Выключение", config.TranslateShutdown || textConstants.shutdown, sddm.canPowerOff]
            ...
            states: [
                State {
                    ...
                    PropertyChanges {
                        ...
                        palette.buttonText: Qt.lighter("#00b988", 1.1)
                    }
                    PropertyChanges {
                        target: parent.children[index].background
                        border.color: Qt.lighter("#00b988", 1.3)
                    }
                },
                State {
                    name: "hovered"
                    ...
                    PropertyChanges {
                        target: parent.children[index]
                        palette.buttonText: Qt.lighter("#00b988", 1.5)
                    }
                    PropertyChanges {
                        target: parent.children[index].background
                        border.color: Qt.lighter("#00b988", 1.7)
                    }
                },
                State {
                    name: "focused"
                    ...
                    PropertyChanges {
                        target: parent.children[index]
                        palette.buttonText: Qt.lighter("#00b988", 1.3)
                    }
                    PropertyChanges {
                        target: parent.children[index].background
                        border.color: Qt.lighter("#00b988", 1.5)
                    }
                }
            ]
